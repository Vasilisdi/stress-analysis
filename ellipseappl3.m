...
numellipse=10;                          %data 1 - No circles/ellipses
...
%---------------------meshing----------------------------------------------
%reg1
h1=50; xbound=85; y1=d/2; maxcoeffellipse=(47.5/y1)^(1/3);
h2=85; ybound=50; thcr=0.685; check=0; scale1=1/100; scale2=1/100;
yin=(linspace(1,maxcoeffellipse,numellipse).^(3))*y1; b=yin;
i=1;  [c r]=size(yin); th=0:step:pi/2;
a=linspace(1,(82.5/47.5),numellipse).*yin; Elem1=[]; l=0; ll=[];
for theta=th
    phi=pi/2-theta;
    x(:,i)=a*cos(phi);
    y(:,i)=b*sin(phi);
...
[cn1,rn1]=size(x);xx=x(1:(cn1),:); yy=y(1:(cn1),:); 
NodeID=[[1:length(reshape(xx,1,(cn1)*(rn1))')]' reshape(xx,1,(cn1)*(rn1))' reshape(yy,1,(cn1)*(rn1))'];
[cx rx]=size(x); itemp=1; 
for j=1:length(th)-1
 for i=1:cx-1
     if j==1
Elem1=[Elem1;i i+cx i+1;i+cx i+1+cx i+1 ];     
     end
     if j~=1
Elem1=[Elem1;i+cx*(j-1) i+cx*j i+1+cx*(j-1) ; i+cx*j i+1+cx*j i+1+cx*(j-1)  ];
     end
...
for i=2:length(th)-1
    ...
    if th(i)==thcr
        ...
        y2=[y2 linspace(y(cx,i),xbound*tan((pi/2)-th(i)),length(x(cx,i):difx:xbound))  xbound*tan((pi/2)-th(i))];
        nth(i)=length(x2);
     ...
     if th(i)<thcr
        ...
        x2=[x2 x(cx,i):difx:(difx/dify)*(ybound-(y(cx,i)-(dify/difx)*x(cx,i)))   (difx/dify)*(ybound-(y(cx,i)-(dify/difx)*x(cx,i)))];
        nth(i)=length(x2);
     ...
     if th(i)>thcr
        x2=[x2 x(cx,i):difx:xbound  xbound];
        y2=[y2 y(cx,i):dify:(dify/difx)*xbound+(y(cx,i)-(dify/difx)*x(cx,i))  (dify/difx)*xbound+(y(cx,i)-(dify/difx)*x(cx,i))];
        nth(i)=length(x2);
        if ( x2(length(x2))-x2(length(x2)-1)<1) %&& th(i)<(pi/2)*0.9 y2(length(x2))-y2(length(x2)-1)<1 %||
          saveempos(j)=length(x2)-1;
          j=j+1;
          nodout=[nodout i];
        end
...
%plot(x2,y2,'ro')
for i=1:length(nodout)
    nth=[nth(1:nodout(i)-1) [nth(nodout(i):length(nth))]-1 ] ;  
    %αφαιρώ για κάθε ακτίνα και αποτις επόμενες της τον αριθμό των αφαιρούμενων κόμβων
end
n=nth;
for i=1:length(n)
    n(i:length(n))=n(i:length(n))-1 ;  
end
n=n+1; diff=100;diff1=200;
x2ere=[];
for j=2:length(th)-2
...
x2me=x2;y2me=y2;x2me(x2ere)=[];y2me(x2ere)=[];
for i=2:length(th)-1
...
x2me=x2;y2me=y2;x2me(x2ere)=[];y2me(x2ere)=[];
%plot(x2me,y2me,'go');
NodeID=[NodeID ;[(cn1)*(rn1)+(1:n(length(n)-1))]' x2me((1:length(x2me)-2))' y2me((1:length(x2me)-2))'];
...
for j=2:length(th)-2
    if th(j)<thcr
            ...
            for ii=1:(nth(j)-nth(j-1))-1
              i=ii+nth(j-1); in=(ii)+n(j-1);
              if ii==1
              Elem2=[Elem2; cx*j cx*(j+1) ((cn1)*(rn1))+in ; cx*(j+1)  ((cn1)*(rn1))+in+n(j)-n(j-1)  ((cn1)*(rn1))+in  ];
              end
              if ii~=1
              Elem2=[Elem2;((cn1)*(rn1))+in-1   ((cn1)*(rn1))+in-1+n(j)-n(j-1) ((cn1)*(rn1))+in ; ((cn1)*(rn1))+in-2+nth(j)-nth(j-1) ((cn1)*(rn1))+in-1+nth(j)-nth(j-1)  ((cn1)*(rn1))+in ];
              end
            ...
            for ii=1:(nth(j)-nth(j-1))-1
              ...
              ie=ie+1;
              Elem2=[Elem2;   ((cn1)*(rn1))+n(j+1)-1 ((cn1)*(rn1))+n(j+1)  ((cn1)*(rn1))+n(j) ];
              end
                if nth(j+1)-nth(j)-nth(j)+nth(j-1)>1
      ...
      %απο εδώ και κάτω για διάστημα θ=[45,90]     
           ...
           if nth(j)-nth(j-1)==nth(j+1)-nth(j)
            ...
            for ii=1:(nth(j+1)-nth(j))-1
              ...
              yevenelemreg2(ie,:)=[y2(nth(j+1)-1) y2(nth(j+1)) y2(nth(j))];
              ie=ie+1;
              Elem2=[Elem2;  ((cn1)*(rn1))+n(j+1) ((cn1)*(rn1))+n(j) ((cn1)*(rn1))+n(j)-1 ];
          ...
          if nth(j)-nth(j-1)==nth(j+1)-nth(j)
            ...
            for ii=1:(nth(j+1)-nth(j))-1
              ...
              if ii~=1
              Elem2=[Elem2;((cn1)*(rn1))+in   ((cn1)*(rn1))+in+n(j)-n(j-1) ((cn1)*(rn1))+in+1 ; ((cn1)*(rn1))+in+nth(j)-nth(j-1) ((cn1)*(rn1))+in+1+nth(j)-nth(j-1)  ((cn1)*(rn1))+in+1 ];
              end
              xoddelemreg2(ie,:)=[x2(i) x2(i+(nth(j)-nth(j-1))) x2(i+1)];
              xevenelemreg2(ie,:)=[x2(i+(nth(j)-nth(j-1))) x2(i+(nth(j)-nth(j-1))+1) x2(i+1) ];
              yoddelemreg2(ie,:)=[y2(i) y2(i+(nth(j)-nth(j-1))) y2(i+1)];
...
x22=[ x(cx,1) 0 ]; y22=[y(cx,1) ybound];%plot(x22,y22,'ko')          %για θ = 0
x22e=[ x(cx,length(th)) xbound ]; y22e=[y(cx,length(th)) 0];%plot(x22e,y22e,'ko')      %για θ = 90
...
x2h=x2;y2h=y2;
%xevenelemreg2=[xoddelemreg2 ;  x2h(nth(holddati+1)) xbound  x2h(nth(holddati))];
%yevenelemreg2=[yoddelemreg2;   y2h(nth(holddati+1)) ybound   y2h(nth(holddati))];

X=[reshape(x,1,cn1*rn1)';[x22 x2 x22e]'];Y=[reshape(y,1,cn1*rn1)';[y22 y2 y22e]'];

Xodd=[xoddelemreg1 ; xoddelemreg2]; Yodd=[yoddelemreg1 ; yoddelemreg2];

Xeven=[xevenelemreg1;xevenelemreg2];Yeven=[yevenelemreg1;yevenelemreg2];
%φτιάχνω την πλάκα για τα 4 τεταρτημόρια
Xodd=[Xodd ; -Xodd ; -Xodd ; Xodd];Yodd=[Yodd ; Yodd ; -Yodd ; -Yodd];
Xeven=[Xeven ; -Xeven ; -Xeven ; Xeven];Yeven=[Yeven ; Yeven ; -Yeven ; -Yeven];
X=[X ; -X ; -X ; X];Y=[Y ; Y ; -Y ; -Y];


numelem=length(Xodd)+length(Xeven);

Elem=[Elem1 ; Elem2];
NodeID=[NodeID ; NodeID(length(NodeID),1)+1   xbound  y2(length(x2));  NodeID(length(NodeID),1)+2  xbound 0  ; NodeID(length(NodeID),1)+3  0  ybound];

Elem=[Elem ; [cx*rx  NodeID(length(NodeID),1)-1 NodeID(length(NodeID),1)-2 ];...
    [cx*rx  NodeID(length(NodeID),1)-2  cx*(rx-1)];[cx*(rx-1) NodeID(length(NodeID),1)-2 NodeID(length(NodeID),1)-3];...
    [cx 2*cx  NodeID(length(NodeID),1)];...
    [2*cx (cx*rx)+1 NodeID(length(NodeID),1) ]];
...
for i=1:length(Nodes)
  ...
  for j=1:size(Elements,1)
   if (Elements(j,1)==Nodes(i,1) || Elements(j,2)==Nodes(i,1) || Elements(j,3)==Nodes(i,1))
       sigx(i)=sigx(i)+tension(1,j);sigxxx=sigx;sigy(i)=sigy(i)+tension(2,j);tau(i)=tau(i)+tension(3,j);
...
nod=fopen('AEM5380_2nd_A_ResultsBending_MeshingEllipse.txt','w+t');
...
sigmanominal1=6*My*d/(t*(100^3-d^3));sigmanominal1edge=6*My*100/(t*(100^3-d^3));
holdsimgamaVM11=vonmises(find(Nodes(find(Nodes(:,2)==0),3)==(d/2)));
%holdsimgamax11=sigx(find(Nodes(find(Nodes(:,2)==0),3)==13.3));
[holdsimgamax11,maxcoorposition]=max(sigx(find(Nodes(:,2)==0),1));
holdcoordsofsym=Nodes(find(Nodes(:,2)==0),3);positionofmaxsigma=holdcoordsofsym(maxcoorposition);
Ktttkampsh=holdsimgamaVM11/sigmanominal1;
giathnkampsh=[holdsimgamax11,positionofmaxsigma];
giathnkampshmaxstr50kai133=[sigx(find(Nodes(find(Nodes(:,2)==0),3)==50)),sigx(find(Nodes(find(Nodes(:,2)==0),3)==(d/2)))];
giathnkampsh1=[sigx(find(Nodes(:,2)==0),1)  , holdcoordsofsym];
%%
...
nod=fopen('AEM5380_2nd_A_ResultsTensile_MeshingEllipse.txt','w+t');
fprintf(nod,  'Maximum_Value #Code_of_NODE      X[mm]    Y[mm]  \n');fprintf('\n');
fprintf(nod,'  sigma_{x}[N/mm2] \n');fprintf('\n');
fprintf(nod,'  %f         %.0f        %f     %f  \n',[ fpr(1,:) ] );fprintf('\n');
fprintf(nod,'  sigma_{y}[N/mm2] \n');fprintf('\n');
...
holdsimgamaVM1=vonmises(find(Nodes(find(Nodes(:,2)==0),3)==(d/2)));
%holdsimgamax1=sigx(find(Nodes(find(Nodes(:,2)==0),3)==13.3));
[holdsimgamax1,maxcoorposition1]=max(sigx(find(Nodes(:,2)==0),1));
holdcoordsofsym1=Nodes(find(Nodes(:,2)==0),3);positionofmaxsigma1=abs(holdcoordsofsym1(maxcoorposition1));
Ktttefelkusmo=holdsimgamaVM1/sigmanominal
giatonefelkusmo=[holdsimgamax1,positionofmaxsigma1];
giatonefelkusmo1=[sigx(find(Nodes(:,2)==0),1)  , holdcoordsofsym1];